...
class CompanyListPagingSource @Inject constructor(
    ...
    override suspend fun load(params: LoadParams<QuerySnapshot>): LoadResult<QuerySnapshot, CommonData> {
        return try {
            ...
            var query: Query = db.collection("companies")
                .orderBy("companyName")  // Ordering by userId
            ...
            val userList = currentPage.documents.map { document ->
                CommonData(
                    id = document.getString("companyId"),
                    name = document.getString("companyName"),
